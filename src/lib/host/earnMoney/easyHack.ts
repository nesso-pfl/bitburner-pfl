...
export const easyHack = async (ns: NS, host: Host, hackStrategy: HackStrategy) => {
  ...
  await repeat(
    ...
    async (count) => {
      ...
      ns.sleep(mws);
      ns.exec(filePath.script.weakenDaemon.$path, weakenServers[count].name, weakenThreads, host);
      ns.sleep(mgs - mws);
      ns.exec(filePath.script.growDaemon.$path, growServers[count].name, growThreads, host);
    },
    th + mh,
